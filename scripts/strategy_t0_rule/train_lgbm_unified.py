...
FEAT_COLS = [
    # classic组
    "dif_norm", "dea_norm", "macd_norm", "macd_area_norm", "dif_slope3_norm",
    "dif_std_norm", "dea_std_norm", "macd_std_norm", "kdj_k_norm", "kdj_d_norm",
    "kdj_j_norm", "kdj_kd_norm", "boll_pct_b_norm", "boll_bw_norm",
    "rsv10_norm", "rsv20_norm",
    # price_volume组
    "vol_ratio_norm", "buy_pressure_norm", "amplitude_norm",
    "ret_1bar_norm", "ret_3bar_norm", "ret_5bar_norm", "range_pos_norm",
    # alpha158组
    "vwap_bias_norm", "x2_norm", "kmid_norm", "kup2_norm", "klow2_norm",
    "cord20_norm", "wvma10_norm", "vsumd10_norm",
]
...
def split_by_cutoff(df: pd.DataFrame, cutoff: pd.Timestamp):
    ...
    logger.info(f"训练集日期: {train_dates[0]} ~ {train_dates[-1]}（{len(train_dates)}天）")
    logger.info(f"测试集日期: {test_dates[0]} ~ {test_dates[-1]}（{len(test_dates)}天）")
...
def train_and_eval(df: pd.DataFrame, label_type: str) -> dict:
    ...
    # 保存模型和结果
    model_path = OUTPUT_DIR / f"lgbm_reg_{label_type}.pkl"
    ...
    result_path = OUTPUT_DIR / f"lgbm_result_{label_type}.csv"
...
def main(label_type: str) -> None:
    ...
    # 可视化
    visualize_model_logic(res["model"], label_type, FEAT_COLS, LGBM_PARAMS, OUTPUT_DIR)

    # 分层收益分析
    csv_path = OUTPUT_DIR / f"lgbm_result_{label_type}.csv"
    if csv_path.exists():
        quantile_return_analysis(csv_path, ret_col=f"fwd_ret_{HOLD_PERIOD}", output_dir=OUTPUT_DIR)
    else:
